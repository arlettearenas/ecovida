...
if (isset($_POST["id_estudio"])) { //Se verifica si se a enviado la variable "id_usuario" por metodo POST.
    ...
    $stmt = $conexion ->prepare("SELECT * FROM estudio WHERE id = '".$_POST["id_estudio"]."' LIMIT 1");
    ...
    foreach($resultado as $fila){
        ...
        $salida["abreviatura"] = $fila["abreviatura"];
        $salida["toma"] = $fila["toma"];
        $salida["frecuencia"] = $fila["frecuencia"];
        $salida["proceso"] = $fila["proceso"];
        $salida["muestra"] = $fila["muestra"];
